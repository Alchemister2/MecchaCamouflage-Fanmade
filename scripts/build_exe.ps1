...
if (-not $OutDir) {
    $OutDir = Join-Path $RuntimeRoot ".build\dist"
}
$RequirementFile = Join-Path $RuntimeRoot "requirements.txt"
if (-not $VenvDir) {
    $VenvDir = Join-Path $RuntimeRoot ".build\venv"
}
...
try {
    $NativeBin = Join-Path $RuntimeRoot ".build\native\bin"
    $NativeInjector = Join-Path $NativeBin "meccha-xenos-injector.exe"
    $NativeBridge = Join-Path $NativeBin "meccha-xenos-bridge.dll"
    if ($BuildNative) {
        ...
        & $BuildNativeScript -RuntimeRoot $RuntimeRoot -OutDir $NativeBin
        if ($LASTEXITCODE -ne 0) {
    ...
    $BuildWorkDir = Join-Path $RuntimeRoot ".build\pyinstaller\work"
    $SpecDir = Join-Path $RuntimeRoot ".build\pyinstaller\spec"
    ...
    if (Test-Path $NativeBin) {
        ...
        $PyInstallerArgs += @("--add-data", "$NativeBin$($DataSeparator)native/bin")
    }
